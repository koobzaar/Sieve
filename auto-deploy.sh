...
REPO_DIR=${SIEVE_REPO_DIR:-$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)}
DOCKER_CONTEXT=${SIEVE_DOCKER_CONTEXT:-$(docker context show)}
OTHER_DOCKER_CONTEXT=${SIEVE_OTHER_DOCKER_CONTEXT:-}
cd "$REPO_DIR"
...
if [ -n "$OTHER_DOCKER_CONTEXT" ] && [ "$OTHER_DOCKER_CONTEXT" != "$DOCKER_CONTEXT" ]; then
...
docker --context "$DOCKER_CONTEXT" compose build sieve
docker --context "$DOCKER_CONTEXT" compose run --rm --no-deps sieve validate-config --runtime
docker --context "$DOCKER_CONTEXT" compose up -d --wait --wait-timeout 240 sieve
